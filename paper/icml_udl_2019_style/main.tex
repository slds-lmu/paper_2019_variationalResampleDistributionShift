...
\icmltitlerunning{ICML 2019 Workshop on Uncertainty and Robustness in Deep Learning}
...
\icmlauthor{Xudong Sun}{equal,lmu}
\icmlauthor{Yu Wang}{equal,tum}
\icmlauthor{Bernd Bischl}{lmu}
\end{icmlauthorlist}

\icmlaffiliation{lmu}{Department of Statistics, University of Munich, Munich, Germany}
\icmlaffiliation{tum}{Technical University of Munich}
\icmlcorrespondingauthor{Xudong Sun}{smilesun.east@gmail.com}
...
Since high dimensional clustering is challenging, we first train a representation of the dataset, either on the whole dataset or with respect to each class label. According to \cite{zhang2013domain}, suppose $X$ represent feature and $Y$ represent the target, we investigate two kinds of distribution shift: The conditional shift which refers to change in $P_{X|Y	}$
