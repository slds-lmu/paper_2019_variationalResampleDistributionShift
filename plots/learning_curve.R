...
# "diagnostics_Bayes3conv3fc_mnist_cv[i]_[test/train]_[rand/].txt"

#--- parse the data

Bayes3conv3fc_mnist_cv_df <- data_frame()
for (i in 0:5) {
  for (train_or_test in c("train", "test")) {
    for (rand in c("CV", "VGMM-CV")) {
      if (rand == "CV") {
        filename <- paste0("./data/diagnostics_Bayes3conv3fc_mnist_cv",
      ...
      if ( !( all(sort(cv_df$epoch) == 1:30) | length(cv_df) < 30) ) {
        stop(paste0("Unexpected number of repeted epochs for CV #",
                    i, ", ", train_or_test, " set, ",
                    "and rand = ", rand))
      }

      Bayes3conv3fc_mnist_cv_df <- bind_rows(Bayes3conv3fc_mnist_cv_df,
                                             cv_df)
    }
  }
}
