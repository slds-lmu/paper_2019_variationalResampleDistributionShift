...
def train(epoch,trainset,inputs,net,batch_size,trainloader,resize,num_epochs,use_cuda,vi,logfile,args):
    net.train()
    train_loss = 0
    correct = 0
    total = 0
...
def test(epoch,testset,inputs,batch_size,testloader,net,use_cuda,num_epochs,resize,vi,logfile,file_name,args):
    global best_acc
    net.eval()
    test_loss = 0
    correct = 0
...
def cross_validation_parallel(i,args,global_rand_idx):
...
def cross_validation_for_clustered_data_parallel(i,args):
    print("cross validation {} for clustered data".format(i))
    num_cluster = config_parent.num_clusters
    # args = parser.parse_args()
    resize = cf.resize
    results = {}
...
if __name__ == '__main__':
    ...
    if args.cv_type == "vgmm":
        ...
        processes = []
        i = 0
        for rank in range(config_parent.num_clusters):
            p = mp.Process(target=cross_validation_for_clustered_data_parallel, args=(i,args))
            i = i + 1
        ...
        #     result = p.map(cross_validation_for_clustered_data_parallel,list(range(config_parent.num_clusters)))
        ...
        for train_eval_idx, test_idx in kf.split(X, y):
            key = key +1
            global_rand_idx[str(key)] = {"train_eval_idx":train_eval_idx,"test_idx":test_idx}
        for rank in range(config_parent.num_clusters):
            i = i + 1
            p = mp.Process(target=cross_validation_parallel, args=(i, args,global_rand_idx))
            # We first train the model across `num_processes` processes
            p.start()
            processes.append(p)
        ...
        for p in processes:
            p.join()
            # with MyPool(multiprocessing.cpu_count()) as p:
            #     result = p.map(cross_validation_parallel, list(range(config_parent.num_clusters)))
